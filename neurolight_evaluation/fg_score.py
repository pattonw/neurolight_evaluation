...
def grid_to_graph(
    ...
    adj_mat = sklearn.feature_extraction.image.grid_to_graph(n_x=s[0], n_y=s[1], n_z=s[2], mask=skeleton)
    # Identify order of the voxels
    voxel_locs = compute_voxel_locs(mask=skeleton)

    G = nx.Graph()

    nodes = [(node_id, {'location': voxel_loc}) for node_id, voxel_loc in enumerate(voxel_locs)]
...
def _make_edges_3d(
    n_x: int, n_y: int, n_z: int, connectivity=26
    ):
